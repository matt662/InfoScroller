...
START_ADDRESS = parser.get('google_maps', 'start_address')
END_ADDRESS = parser.get('google_maps', 'end_address')
                ...
                for cc in chromecasts:
                    if cc.media_controller.status.player_is_playing:
                        if cc.media_controller.status.artist and cc.media_controller.status.title:
                            message = {'now_playing': {'artist': cc.media_controller.status.artist,
                                                       'title': cc.media_controller.status.title}}
                        else:
                            message = {'now_playing': {'title': cc.media_controller.status.title}}
                        logging.info('added message for chromecast')
                        self.q.put(message)
            except:
                logging.warning('General Error on Chromecast Fetch')
                traceback.print_exc()
            for i in range(10):
                if interrupt:
